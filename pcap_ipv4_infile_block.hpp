...
#ifndef PCAP_IPV4_INFILE_BLOCK_HPP
#define PCAP_IPV4_INFILE_BLOCK_HPP
...
//////////////////Class definition
class Pcap_IPv4_Infile_Block: public Processing_Block{
  private:
    pcap_t *pcap_descr;
  ...
  public:
    static void call_back_entry_point(u_char* inval,const struct pcap_pkthdr* pkthdr, const u_char* packet){
       ((Pcap_IPv4_Infile_Block *)inval)->entry_point2(pkthdr,packet);
    };
    ...
    int get_print_stats();
    Pcap_IPv4_Infile_Block();
    ~Pcap_IPv4_Infile_Block();
};

////////Class implementation
Pcap_IPv4_Infile_Block::~Pcap_IPv4_Infile_Block(){
    //get_print_stats();
    if (NULL!=pcap_descr){
...
int Pcap_IPv4_Infile_Block::initialize(int in_numoutputs,char *in_filename){
   return initialize(in_numoutputs,in_filename,0);
};

int Pcap_IPv4_Infile_Block::initialize(int in_numoutputs,char *in_filename,int type){
  int i;
  ...
  if(0==type)
     ...
     pcap_descr = pcap_open_live(in_filename,1500,1,-1,errbuf);
     live_read=true;
  }
  if(pcap_descr == NULL)
...
int Pcap_IPv4_Infile_Block::entry_point(const Tagged_IP_Packet *in_packet){
...
int Pcap_IPv4_Infile_Block::entry_point2(const struct pcap_pkthdr* pkhdr,const u_char *pkt_data){
  int i;
  int offset;
...
int Pcap_IPv4_Infile_Block::set_output_point(Processing_Block *out_block){
...
int Pcap_IPv4_Infile_Block::get_print_stats(){
...
// if(last_loop_stats.ps_drop!=0){
     cout  << "recv="<< last_loop_stats.ps_recv 
           <<" drop="<<last_loop_stats.ps_drop 
     //cout <<" if drop"<< last_loop_stats.ps_ifdrop ;
           << " it_recv=" <<last_loop_stats.ps_recv-last_stats.ps_recv 
           << " it_drop=" <<last_loop_stats.ps_drop-last_stats.ps_drop 
           << " capture ratio(total) =" <<(1.0-last_loop_stats.ps_drop/(1.0*last_loop_stats.ps_recv))
           << endl;
//}
  last_stats=last_loop_stats;
  return 0;
};
