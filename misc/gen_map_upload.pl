...
#-----
#----- Version:  $Id: gen_map_upload.pl 1757 2005-07-15 16:30:26Z cvs $
#-----
#----- Authors:  Edward Balas <ebalas@iu.edu>
#                Hacked by Camilo Viecco on 2007 for uploading the generator list.
...
sub main{ 
    ...
    my $query = "insert into ids_sig (sensor_id,ids_sig_gen,ids_sig_id, sig_name) values (?,?,?,?) ";
	...
	($gen_id,$sid,$msg) = split(/ \|\| /,$_,3);	

        #print "gen=$gen_id sid=$sid msg=$msg \n";
        if (defined $gen_id){
	  $sql->execute($sensor_id,$gen_id,$sid,$msg);
        }
    }
};
